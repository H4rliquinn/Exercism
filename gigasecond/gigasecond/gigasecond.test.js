...
const gigasecond_1 = __importDefault(require("./gigasecond"));
describe('Gigasecond', () => {
    it('tells a gigasecond anniversary since midnight', () => {
        const gs = new gigasecond_1.default(new Date(Date.UTC(2015, 8, 14)));
    ...
    xit('tells the anniversary is next day when you are born at night', () => {
        const gs = new gigasecond_1.default(new Date(Date.UTC(2015, 8, 14, 23, 59, 59)));
        const expectedDate = new Date(Date.UTC(2047, 4, 24, 1, 46, 39));
        expect(gs.date()).toEqual(expectedDate);
    });
    xit('even works before 1970 (beginning of Unix epoch )', () => {
        const gs = new gigasecond_1.default(new Date(Date.UTC(1959, 6, 19, 5, 13, 45)));
...
//# sourceMappingURL=gigasecond.test.js.map